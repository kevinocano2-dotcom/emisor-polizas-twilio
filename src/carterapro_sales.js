...
export function combinedHelp() {
  return [
    ...
    '',
    'Si Kevin ya tomó tu conversación, escribe CARTERAPRO para volver a activar el asistente.'
  ].join('\n');
}
...
export async function handleCarteraProMessage({ from, body, baseUrl, notifyAdmin = async () => {}, adContext = null }) {
  ...
  if (isOneOf(x,'SALIR CARTERAPRO','CERRAR CARTERAPRO')) {
    ...
    return { body:'Listo. Cerré el flujo de CarteraPro. Escribe CARTERAPRO cuando quieras volver a empezar.', source:'carterapro_sales' };
  }
