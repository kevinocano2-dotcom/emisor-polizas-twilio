...
let firebaseDb = null;
let firebaseBackend = 'realtime';
let firebaseReady = false;
let firebaseDisabledReason = '';
let firebaseLastErrorAt = '';

function firebaseErrorText(err) {
  return String(err?.details || err?.message || err || 'Error Realtime Database').slice(0, 300);
}

function noteFirebaseError(err, action = 'Realtime Database') {
  firebaseDisabledReason = `Realtime Database error: ${firebaseErrorText(err)}`;
  firebaseLastErrorAt = new Date().toISOString();
  // IMPORTANTE: no apagamos Firebase por un error aislado. El siguiente mensaje vuelve a intentar.
  // Si el guardado falla, recordMessage mantiene un respaldo local y la sincronización con Twilio
  // puede reconstruir mensajes faltantes después.
  console.error(`${action}: error. Se conserva la conexión para reintentar en la siguiente operación.`, err?.message || err);
}

function noteFirebaseSuccess() {
...
export function firebaseStatus() {
  return {
    ...
    firebaseBackend: firebaseDb ? 'realtime' : 'local',
    firebaseDisabledReason,
    firebaseLastErrorAt,
    dataBackend: firebaseDb ? 'realtime' : 'local'
  };
}
...
function requestedBackend() {
  // CarteraPro usa exclusivamente Firebase Realtime Database.
  return 'realtime';
}
...
function initFirebase() {
  ...
  if (!raw) {
    console.warn('Realtime Database no configurada. Usando respaldo local temporal.');
    return false;
  }
  ...
  try {
    ...
    const databaseURL =
      ...
      (parsed.project_id ? `https://${parsed.project_id}-default-rtdb.firebaseio.com` : '');

    if (!databaseURL) {
      console.warn('No se pudo determinar FIREBASE_DATABASE_URL. Usando respaldo local temporal.');
      return false;
    }
    ...
    firebaseBackend = 'realtime';
    firebaseDb = admin.database();
    noteFirebaseSuccess();
    console.log(`Firebase Realtime Database conectado: ${databaseURL}`);
    return true;
  } catch (err) {
    console.error('No se pudo iniciar Realtime Database. Usando respaldo local temporal:', err.message);
    firebaseDb = null;
    firebaseBackend = 'realtime';
    firebaseDisabledReason = `Realtime Database init error: ${firebaseErrorText(err)}`;
    firebaseLastErrorAt = new Date().toISOString();
    return false;
  }
}
...
export function storageMode() {
  return useFirebase() ? 'firebase_realtime_database' : 'local_backup';
}
...
async function fbGetDoc(collectionName, id) {
  ...
  try {
    ...
    noteFirebaseError(err, `RTDB get ${collectionName}`);
    throw err;
  }
}

async function fbSetDoc(collectionName, id, data, { merge = true } = {}) {
  if (!useRealtime()) throw new Error('Realtime Database no disponible');
  const docId = safeKey(id);
  try {
    ...
    noteFirebaseError(err, `RTDB set ${collectionName}`);
    throw err;
  }
}
...
async function fbDeleteDoc(collectionName, id) {
  ...
  try {
    ...
    noteFirebaseError(err, `RTDB delete ${collectionName}`);
    throw err;
  }
}
...
async function fbListDocs(collectionName, { limit = 500, orderBy = '', orderDirection = 'desc' } = {}) {
  ...
  try {
    ...
    if (orderBy) rows.sort((a,b) => {
      const av = Number(a?.[orderBy] || 0), bv = Number(b?.[orderBy] || 0);
      return orderDirection === 'asc' ? av-bv : bv-av;
    });
    return rows.slice(0,max);
  } catch (err) {
    noteFirebaseError(err, `RTDB list ${collectionName}`);
    throw err;
  }
}
...
async function updateConversationIndex(record) {
  ...
  const hadMessages=Number(previous.totalMessages||0)>0;
  const isLatest=!hadMessages || createdMs>=Number(previous.lastAtMs||0);
  const isEarliest=!hadMessages || createdMs<Number(previous.firstAtMs||createdMs);
  const readAtMs=previous.readAt ? new Date(previous.readAt).getTime() : 0;
  const unreadDelta=isInbound && (!readAtMs || createdMs>readAtMs) ? 1 : 0;
  const newerInbound=isInbound && createdMs>=Number(previous.lastInboundAtMs||0);
  const newerOutbound=isOutbound && createdMs>=Number(previous.lastOutboundAtMs||0);

  const next = normalizeConversation({
    ...previous,
    phone: clean,
    area: conversationAreaFromSource(record.source, previous.area),
    firstAt: isEarliest ? record.createdAt : previous.firstAt,
    firstAtMs: isEarliest ? createdMs : previous.firstAtMs,
    lastAt: isLatest ? record.createdAt : previous.lastAt,
    lastAtMs: isLatest ? createdMs : previous.lastAtMs,
    lastBody: isLatest ? record.body : previous.lastBody,
    lastDirection: isLatest ? record.direction : previous.lastDirection,
    lastSource: isLatest ? record.source : previous.lastSource,
    ...
    unreadCount: Number(previous.unreadCount || 0) + unreadDelta,
    lastInboundAt: newerInbound ? record.createdAt : previous.lastInboundAt,
    lastInboundAtMs: newerInbound ? createdMs : previous.lastInboundAtMs,
    lastOutboundAt: newerOutbound ? record.createdAt : previous.lastOutboundAt,
    lastOutboundAtMs: newerOutbound ? createdMs : previous.lastOutboundAtMs,
  ...
  if (useFirebase()) {
    try {
      ...
      console.error('No se pudo guardar índice de conversación en Realtime Database:', err?.message || err);
    }
  }
...
export async function recordMessage({ phone, direction, source = '', body = '', from = '', to = '', sid = '', mediaUrl = '', status = '', referral = null, createdAt = '', createdAtMs = 0 }) {
  const clean = normalizePhone(phone || from || to);
  const key = messageKeyFrom({sid});

  // Idempotencia por SID: el webhook y la reparación desde Twilio pueden ver el mismo mensaje.
  // Si ya existe, NO volvemos a incrementar contadores ni a duplicar la conversación.
  if (clean && useRealtime()) {
    const existing = await getStoredChatMessage(clean,key,sid);
    if (existing) return { ...existing, storedIn:'firebase_realtime_database', duplicate:true };
  }

  const tsMs = Number(createdAtMs || 0) || Date.now();
  const tsIso = createdAt ? toIso(createdAt) : new Date(tsMs).toISOString();

  // Las respuestas hechas con TwiML se guardan primero sin SID. Cuando Twilio las devuelve
  // en Message Logs, vinculamos el SID al registro existente en lugar de duplicarlo.
  if(clean && sid && useRealtime()){
    const match=await findMatchingUnsidedMessage(clean,{direction,body,createdAtMs:tsMs});
    if(match){
      await firebaseDb.ref(`chat_messages/${safeKey(clean)}/${safeKey(match.id)}`).update({sid:String(sid),status:String(status||match.status||'')});
      await firebaseDb.ref(`message_sid_index/${safeKey(sid)}`).set(match.id);
      return {...match,sid:String(sid),status:String(status||match.status||''),storedIn:'firebase_realtime_database',duplicate:true,linkedSid:true};
    }
  }
  ...
  if (clean && useRealtime()) {
    try {
      await firebaseDb.ref(`chat_messages/${safeKey(clean)}/${safeKey(record.id)}`).set(record);
      if(record.sid) await firebaseDb.ref(`message_sid_index/${safeKey(record.sid)}`).set(record.id);
      noteFirebaseSuccess();
      storedIn = 'firebase_realtime_database';
    } catch (err) {
      noteFirebaseError(err, 'recordMessage RTDB');
      record.firebaseSaveError = firebaseErrorText(err);
    }
  }

  if (storedIn !== 'firebase_realtime_database') {
    const messages = readJson(MESSAGES_FILE, []).map(normalizeMessage);
    if (!localMessageExists(messages,record.id,record.sid)) {
      messages.push(record);
      // Evita que el respaldo temporal crezca sin límite en Render.
      writeJson(MESSAGES_FILE, messages.slice(-10000));
    }
  }
...
function flattenChatRoot(root = {}) {
  const out=[];
  for (const [phone, messages] of Object.entries(root||{})) {
    for (const [id, value] of Object.entries(messages||{})) {
      out.push(normalizeMessage({id,phone,...(value||{})}));
    }
  }
  return out;
}
...
export async function loadMessages({ phone = '', limit = 200 } = {}) {
  ...
  if (useRealtime()) {
    try {
      if (clean) {
        const snap = await firebaseDb.ref(`chat_messages/${safeKey(clean)}`)
          .orderByChild('createdAtMs').limitToLast(max).once('value');
        const value=snap.val()||{};
        remote=Object.entries(value).map(([id,v])=>normalizeMessage({id,phone:clean,...(v||{})}));
      } else {
        // Solo se usa para reconstrucción/manual. El Inbox normal trabaja con conversations.
        const snap=await firebaseDb.ref('chat_messages').once('value');
        remote=flattenChatRoot(snap.val()||{});
      }
      noteFirebaseSuccess();
    } catch (err) {
      noteFirebaseError(err,'loadMessages RTDB');
    }
  }

  // Mezcla cualquier respaldo local del arranque actual. La deduplicación usa SID/ID.
  const local=readJson(MESSAGES_FILE,[]).map(normalizeMessage)
    .filter(m=>!clean || m.phone===clean);
  const map=new Map();
  for (const m of [...remote,...local]) {
    const k=String(m.sid||m.id||`${m.phone}_${m.createdAtMs}_${m.direction}_${m.body}`);
    const prev=map.get(k);
    if(!prev || Number(m.createdAtMs||0)>=Number(prev.createdAtMs||0)) map.set(k,m);
  }
