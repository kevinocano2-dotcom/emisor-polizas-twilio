...
function noteFirestoreError(err, action = 'Firestore') {
  if (isFirestoreQuotaError(err)) {
    ...
    console.error(`${action}: cuota de Firestore agotada. Se evita reiniciar el servidor.`, err.message || err);
  } else {
...
function useFirebase() {
  if (firebaseDisabledReason) return false;
  return initFirebase() && db;
}

export function firestoreStatus() {
  return {
    firebaseReady: Boolean(db) && !firebaseDisabledReason,
...
export async function loadPolicies({ limit = 300 } = {}) {
  if (useFirebase()) {
    try {
...
export async function loadMessages({ phone = '', limit = 200 } = {}) {
  ...
  if (useFirebase()) {
    try {
      ...
      if (clean) {
        // Sin orderBy para evitar índice compuesto. Limitamos lecturas desde Firestore.
        query = query.where('phone', '==', clean).limit(max);
      } else {
        query = query.orderBy('createdAtMs', 'desc').limit(max);
      }
...
export async function getConversationSummary() {
  const messages = await loadMessages({ limit: 200 });
