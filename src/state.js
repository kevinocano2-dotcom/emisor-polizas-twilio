...
function noteFirestoreError(err, action = 'Firestore') {
  if (isFirestoreQuotaError(err)) {
    ...
    console.error(`${action}: cuota de Firestore agotada. Se activará respaldo local de emergencia para no detener WhatsApp.`, err.message || err);
  } else {
    firebaseDisabledReason = 'Firestore error';
    console.error(`${action}: error Firestore. Se activará respaldo local de emergencia para no detener WhatsApp.`, err.message || err);
  }
}

function firestoreErrorText(err) {
  return String(err?.details || err?.message || err || 'Error Firestore').slice(0, 300);
}
...
export async function getLastFolioNumber() {
  ...
  if (useFirebase()) {
    try {
      ...
      return Number(snap.data().last || initial);
    } catch (err) {
...
export async function setLastFolioNumber(n) {
  ...
  if (useFirebase()) {
    try {
      ...
      writeJson(COUNTER_FILE, { last: value, updatedAt: new Date().toISOString(), mirrorOfFirebase: true });
      return;
    } catch (err) {
      noteFirestoreError(err, 'setLastFolioNumber');
    }
  }

  writeJson(COUNTER_FILE, { last: value, updatedAt: new Date().toISOString(), emergencyLocal: true });
}
...
export async function nextFolioNumber() {
  if (useFirebase()) {
    ...
    try {
      ...
      writeJson(COUNTER_FILE, { last: next, updatedAt: new Date().toISOString(), mirrorOfFirebase: true });
      return next;
    } catch (err) {
      noteFirestoreError(err, 'nextFolioNumber');
    }
  }

  const next = await getLastFolioNumber() + 1;
  writeJson(COUNTER_FILE, { last: next, updatedAt: new Date().toISOString(), emergencyLocal: true });
  return next;
}
...
export async function getSession(phone) {
  ...
  if (useFirebase()) {
    try {
      ...
      noteFirestoreError(err, 'getSession');
    }
  }
...
export async function setSession(phone, session) {
  ...
  if (useFirebase()) {
    try {
      ...
      noteFirestoreError(err, 'setSession');
    }
  }

  const sessions = readJson(SESSIONS_FILE, {});
  sessions[clean] = { ...session, phone: clean, updatedAt: new Date().toISOString(), emergencyLocal: Boolean(firebaseDisabledReason) };
...
export async function clearSession(phone) {
  ...
  if (useFirebase()) {
    try {
      await db.collection('sessions').doc(clean).delete();
      return;
    } catch (err) {
      noteFirestoreError(err, 'clearSession');
    }
  }
...
export async function loadUsers() {
  if (useFirebase()) {
    try {
      ...
      noteFirestoreError(err, 'loadUsers');
    }
  }
...
export async function getUser(phone) {
  ...
  if (useFirebase()) {
    try {
      ...
      noteFirestoreError(err, 'getUser');
    }
  }
...
export async function isUserAllowed(phone) {
  ...
  // Si Firestore falló, permite únicamente números de emergencia definidos por variable.
  // Esto evita que el bot quede muerto por cuota, sin abrirlo a todos por defecto.
...
export async function recordPolicy({
  ...
  if (useFirebase()) {
    try {
      ...
      return { ...record, storedIn: 'firebase' };
    } catch (err) {
      noteFirestoreError(err, 'recordPolicy');
      record.firebaseSaveError = firestoreErrorText(err);
...
export async function getPolicyStatsForPhone(phone) {
  ...
  if (useFirebase()) {
    try {
      ...
      const policies = readJson(POLICIES_FILE, []);
      userPolicies = policies.map(normalizePolicy).filter(p => p.phone === clean);
    }
  } else {
    const policies = await loadPolicies();
...
export async function recordAffiliation({
  ...
  if (useFirebase()) {
    try {
      ...
      noteFirestoreError(err, 'recordAffiliation');
      record.firebaseSaveError = firestoreErrorText(err);
      record.storedIn = 'local_backup';
    }
  }
