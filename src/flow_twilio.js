...
function errorSummary(err) {
  ...
  if (/RESOURCE_EXHAUSTED|Quota exceeded/i.test(msg) || err?.code === 8) {
    return 'Realtime Database temporalmente no disponible';
  }
...
function localBackupWarning(record, label) {
  ...
  return `${label} generado, pero NO se guardó en Realtime Database. Quedó en respaldo local temporal. Error: ${record.firebaseSaveError || 'Firebase no disponible'}`;
}
...
Aviso: Realtime Database no guardó la póliza. Ya envié respaldo de datos a administración por WhatsApp.`
            : '';
...
Aviso: Realtime Database no guardó la afiliación. Ya envié respaldo de datos a administración por WhatsApp.`
            : '';
...
EXP / Placa: ${affResult.affiliation.exp}${warningText}`,
        ...
        const warningText = storageWarnings.length
          ? `\n\nAviso: Realtime Database no guardó uno o más registros. Ya envié respaldo de datos a administración por WhatsApp.`
          : '';
