...
const COMMANDS = {
  EMITIR: 'policy',
  EMITIRADMINISTRATIVO: 'policy',
  POLIZA: 'policy',
  SEGURO: 'policy',
  EMITIRYAFILIACION: 'policy_affiliation',
  EMITIRAFILIACION: 'policy_affiliation',
  POLIZAYAFILIACION: 'policy_affiliation',
  AFILIACION: 'affiliation',
  MYR: 'affiliation'
};

const KEYWORD_LABEL = 'EMITIR, EMITIRYAFILIACION o AFILIACION';
const INVALID_COMMAND_TEXT = 'Comando incorrecto. Para iniciar emisión manda la palabra clave: EMITIR. También puedes mandar EMITIRYAFILIACION o AFILIACION.';
    ...
    if (/^CONFIRMAR$/i.test(text)) {
      ...
      try {
        ...
        if (confirmedMode === 'policy') {
          ...
          const stats = await getPolicyStatsForPhone(sender);
          ...
          return {
            body: `Listo. Folio emitido: ${result.policy.folio}\nPendientes de pagar de este usuario: ${pendingText}.`,
