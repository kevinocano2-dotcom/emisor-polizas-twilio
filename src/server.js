...
function twimlMessage({ body, mediaUrl, mediaUrls }) {
  const urls = []
  ...
  const media = urls.map(url => `<Media>${xmlEscape(url)}</Media>`).join('');
...
function buildAdminNotificationText(info = {}) {
  if (info.kind === 'emit_error_backup') {
    ...
    const lines = [
      '⚠️ ERROR AL EMITIR / GUARDAR PÓLIZA',
      `Error: ${info.error || '-'}`,
      `Usuario: ${info.phone || '-'}`,
      '',
    ...
    lines.push('', 'No se guardó en Firebase. Capturar o verificar cuando se recupere Firestore.');
    return lines.join('\n');
  }
