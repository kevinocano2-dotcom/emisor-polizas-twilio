...
import { deleteUser, getAdminSummary, getConversationSummary, getLastFolioNumber, liquidatePolicies, loadMessages, markPoliciesPaid, nextFolioNumber, normalizePhone, recordAffiliation, recordMessage, setLastFolioNumber, setPolicyStatus, setUserActive, storageMode, upsertUser, firebaseStatus, getSession, loadCarteraProLeads, loadCarteraProOnboarding, getCarteraProOnboarding, saveCarteraProOnboarding, storeCarteraProUpload, getFirebaseStorageBucketName, recordCarteraProLead, loadConversationIndex, markConversationRead, rebuildConversationIndex, getCarteraProSession, setCarteraProSession } from './state.js';
...
function twimlMessage(reply = {}) {
  if (reply?.silent) return '<?xml version="1.0" encoding="UTF-8"?><Response></Response>';
  let messages = [];
...
function buildAdminNotificationText(info = {}) {
  ...
  if (info.kind === 'emit_error_backup') {
    ...
    lines.push('', 'Revisar/capturar en Firebase cuando se recupere Realtime Database. Los enlaces temporales dependen de que Render siga corriendo.');
    return lines.join('\n');
  }
...
const twilioSyncState={lastRecentAtMs:0,lastRecentCount:0,lastError:'',running:false};
const inboundAlertCooldown=new Map();

function sameWhatsAddress(a,b){return normalizePhone(a)===normalizePhone(b)}
function twilioMessageOtherPhone(m){
  const ours=normalizePhone(twilioFromAddress());
  const from=normalizePhone(m?.from||''),to=normalizePhone(m?.to||'');
  if(from===ours)return to;
  if(to===ours)return from;
  return '';
}
function twilioMessageDirection(m){
  return sameWhatsAddress(m?.from,twilioFromAddress())?'outbound':'inbound';
}
async function persistTwilioLogMessage(m){
  const phone=twilioMessageOtherPhone(m);
  if(!phone)return null;
  const direction=twilioMessageDirection(m);
  const createdAtMs=new Date(m?.dateCreated||m?.dateSent||Date.now()).getTime();
  const result=await recordMessage({
    ...
    createdAt:m?.dateCreated||m?.dateSent||new Date(),
    createdAtMs
  });
  // Si por un fallo del webhook el mensaje solo apareció en Message Logs de Twilio,
  // lo recuperamos y avisamos al administrador siempre que sea reciente.
  if(direction==='inbound' && result && !result.duplicate && Date.now()-createdAtMs<15*60*1000){
    notifyInboundMessage({phone,body:String(m?.body||'')}).catch(()=>{});
  }
  return result;
}
async function syncTwilioRecent({limit=300,force=false}={}){
  const now=Date.now();
  if(twilioSyncState.running)return {ok:true,skipped:true,reason:'running',...twilioSyncState};
  if(!force && now-twilioSyncState.lastRecentAtMs<60000)return {ok:true,skipped:true,reason:'fresh',...twilioSyncState};
  twilioSyncState.running=true;
  try{
    const client=twilioClient();
    const rows=await client.messages.list({limit:Math.max(20,Math.min(Number(limit||300),1000))});
    let stored=0;
    for(const m of rows){const r=await persistTwilioLogMessage(m);if(r&&!r.duplicate)stored++;}
    twilioSyncState.lastRecentAtMs=Date.now();twilioSyncState.lastRecentCount=rows.length;twilioSyncState.lastError='';
    return {ok:true,fetched:rows.length,stored,lastRecentAtMs:twilioSyncState.lastRecentAtMs};
  }catch(err){
    twilioSyncState.lastError=String(err?.message||err);console.error('No se pudo sincronizar historial Twilio:',twilioSyncState.lastError);
    return {ok:false,error:twilioSyncState.lastError};
  }finally{twilioSyncState.running=false;}
}
async function syncTwilioPhone(phone,{limit=250}={}){
  const clean=normalizePhone(phone);if(!clean)return {ok:false,error:'Número inválido'};
  try{
    const client=twilioClient(), addr=formatWhatsAppAddress(clean);
    const each=Math.max(20,Math.min(Number(limit||250),500));
    const [toRows,fromRows]=await Promise.all([
      client.messages.list({to:addr,limit:each}),
      client.messages.list({from:addr,limit:each})
    ]);
    const map=new Map();for(const m of [...toRows,...fromRows])map.set(m.sid,m);
    let stored=0;for(const m of map.values()){const r=await persistTwilioLogMessage(m);if(r&&!r.duplicate)stored++;}
    return {ok:true,fetched:map.size,stored};
  }catch(err){console.error('No se pudo sincronizar chat Twilio:',err?.message||err);return {ok:false,error:String(err?.message||err)};}
}
function adminInboxUrl(){
  const root=String(process.env.PUBLIC_BASE_URL||'https://emisor-polizas-twilio.onrender.com').replace(/\/$/,'');
  const token=adminTokenExpected();
  return `${root}/carterapro-inbox.html${token?`?token=${encodeURIComponent(token)}`:''}`;
}
function adminChatUrl(phone){
  const root=String(process.env.PUBLIC_BASE_URL||'https://emisor-polizas-twilio.onrender.com').replace(/\/$/,'');
  const token=adminTokenExpected();
  const q=new URLSearchParams({phone:normalizePhone(phone),v:'5'});
  if(token)q.set('token',token);
  return `${root}/carterapro-chat.html?${q.toString()}`;
}
const inboundAlertBurst=new Map();
async function sendInboundAdminAlert(clean, preview, extraCount=0){
  await notifyCarteraProAdmin([
    extraCount>0 ? `🔔 *${extraCount} mensaje${extraCount===1?'':'s'} adicional${extraCount===1?'':'es'} en CarteraPro*` : '🔔 *Nuevo mensaje en CarteraPro*',
    `WhatsApp: ${clean}`,
    `Mensaje: ${preview}`,
    '',
    `💬 Responder desde CarteraPro (mismo número Twilio): ${adminChatUrl(clean)}`,
    `📲 Responder desde mi WhatsApp personal: ${directWhatsAppReplyLink(clean,'Hola, vi tu mensaje de CarteraPro. ¿En qué te puedo apoyar?')}`,
    `📥 Inbox: ${adminInboxUrl()}`
  ].join('\n'));
}
async function notifyInboundMessage({phone,body}){
  const clean=normalizePhone(phone);if(!clean)return;
  if(adminNotifyNumbers().includes(clean))return;
  const preview=String(body||'').trim().replace(/\s+/g,' ').slice(0,220)||'(mensaje sin texto)';
  const now=Date.now();
  let state=inboundAlertBurst.get(clean);
  if(!state || now-Number(state.lastImmediateAt||0)>90000){
    if(state?.timer)clearTimeout(state.timer);
    state={lastImmediateAt:now,pending:0,lastPreview:preview,timer:null};
    inboundAlertBurst.set(clean,state);
    await sendInboundAdminAlert(clean,preview,0);
    return;
  }
  ...
  state.timer=setTimeout(async()=>{
    const cur=inboundAlertBurst.get(clean);
    if(!cur||!cur.pending)return;
    const count=cur.pending,last=cur.lastPreview;
    cur.pending=0;cur.lastImmediateAt=Date.now();cur.timer=null;
    try{await sendInboundAdminAlert(clean,last,count)}catch(e){console.error('No se pudo enviar resumen de mensajes:',e?.message||e)}
  },45000);
}
...
function directWhatsAppReplyLink(phone, text = '') {
  let p = normalizePhone(phone);
  if (p.length === 10) p = `52${p}`;
  if (!p) return '';
  const base = `https://wa.me/${p}`;
  return text ? `${base}?text=${encodeURIComponent(String(text))}` : base;
}

app.get('/carterapro/demo-whatsapp', (req,res)=>{
  const target=adminNotifyNumbers()[0] || normalizePhone(twilioFromAddress());
  if(!target)return res.status(503).send('WhatsApp de demostración no configurado.');
  const clean=(v,max=80)=>String(v||'').replace(/[\r\n<>]/g,' ').trim().slice(0,max);
  const client=clean(req.query.client)||'Kevin';
  const vehicle=clean(req.query.vehicle)||'Kia Optima';
  const model=clean(req.query.model)||'2013';
  const date=clean(req.query.date)||'30 de agosto de 2026';
  const company=clean(req.query.company)||'AXA';
  const policy=clean(req.query.policy)||'';
  const reason=clean(req.query.reason,20).toLowerCase()||'renew';
  let message;
  if(reason==='pay'){
    message=[
      `Hola ${client} 👋`,'',
      `Le escribimos para dar seguimiento al pago pendiente de su póliza ${company}${policy?` No. ${policy}`:''}.`,
      '','Si ya realizó el pago, puede compartirnos su comprobante por este medio; si requiere apoyo, estamos al pendiente.','',
      'Quedamos atentos a sus comentarios.','', '— Mensaje de prueba de CarteraPro'
    ].join('\n');
  }else if(reason==='follow'){
    message=[`Hola ${client} 👋`,'',`Le escribimos para dar seguimiento a su póliza ${company}${policy?` No. ${policy}`:''}.`,'','Estamos al pendiente si requiere apoyo o información adicional.','', 'Quedamos atentos a sus comentarios.','', '— Mensaje de prueba de CarteraPro'].join('\n');
  }else{
    message=[
      `Hola ${client} 👋`,'',
      `Le avisamos que la póliza de su ${vehicle} ${model} está próxima a vencer el ${date}.`,'',
      'Estamos al pendiente para apoyarle con su renovación y, si lo requiere, con el pago. Si gusta, podemos enviarle la información necesaria para continuar.','',
      'Quedamos atentos a sus comentarios.','', '— Mensaje de prueba de CarteraPro'
    ].join('\n');
  }
...
async function buildCarteraProInbox() {
  ...
  }).filter(c=>!adminNotifyNumbers().includes(normalizePhone(c.phone))).sort((a,b)=>b.lastAtMs-a.lastAtMs);
}

app.post('/api/admin/carterapro/sync-twilio', requireAdmin, async (req,res)=>{
  try{
    const phone=normalizePhone(req.body?.phone||req.query?.phone||'');
    const result=phone?await syncTwilioPhone(phone,{limit:Number(req.body?.limit||300)}):await syncTwilioRecent({limit:Number(req.body?.limit||500),force:true});
    res.status(result.ok===false?502:200).json(result);
  }catch(err){res.status(500).json({ok:false,error:err.message||'No se pudo sincronizar Twilio'});}
});

app.get('/api/admin/carterapro/inbox', requireAdmin, async (req,res)=>{
  try{
    await syncTwilioRecent({limit:400}).catch(()=>{});
    ...
    res.json({
      ...
      conversations,
      messageStorage:storageMode(),
      twilioSync:{lastAtMs:twilioSyncState.lastRecentAtMs,lastCount:twilioSyncState.lastRecentCount,lastError:twilioSyncState.lastError}
    });
  }catch(err){
...
app.get('/api/admin/carterapro/thread/:phone', requireAdmin, async (req,res)=>{
  try{
    ...
    await syncTwilioPhone(phone,{limit:300}).catch(()=>{});
...
app.post('/api/admin/messages/reply', requireAdmin, async (req, res) => {
  try {
    ...
    // Cuando Kevin responde desde el Inbox, el bot comercial se pausa para ese cliente.
    // Esto evita que una respuesta humana dispare nuevamente el mismo menú automático.
    const currentSalesSession = await getCarteraProSession(phone).catch(()=>null);
    const humanHandoffUntil = new Date(Date.now() + Math.max(1,Number(process.env.CARTERAPRO_HUMAN_HANDOFF_HOURS||12))*60*60*1000).toISOString();
    await setCarteraProSession(phone,{
      ...(currentSalesSession||{createdAt:new Date().toISOString()}),
      state:'human_handoff',
      data:{...(currentSalesSession?.data||{}),humanHandoffUntil,humanHandoffReason:'respuesta_manual_inbox',handoffNoticeSentAt:new Date().toISOString()}
    }).catch(err=>console.error('No se pudo pausar bot tras respuesta manual:',err?.message||err));
...
function registrationAlertCors(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers','Content-Type, X-Cartera-Name, X-Cartera-Phone, X-Cartera-Email, X-Cartera-Company, X-Cartera-Missing, X-Cartera-File, X-Cartera-Source, X-Cartera-Username, X-Cartera-User-Uid');
...
app.post('/carterapro/api/registration-alert', async (req,res)=>{
  ...
  await notifyCarteraProAdmin([
    ...
    '',
    `💬 Responder al cliente: ${directWhatsAppReplyLink(phone,'Hola, recibí tu registro de CarteraPro. Si necesito confirmar algún dato te escribo por aquí.')}`
  ].filter(Boolean).join('\n'));
  ...
  async (req,res)=>{
    ...
    try{
      ...
      await notifyCarteraProAdmin([
        ...
        '',
        `💬 Responder al cliente: ${directWhatsAppReplyLink(phone,'Hola, recibí tu solicitud para adaptar el formato de póliza en CarteraPro. Ya la estoy revisando y si necesito el PDF o algún dato adicional te aviso por aquí.')}`
      ].filter(Boolean).join('\n'));
...
app.post(['/twilio/webhook', '/webhook'], async (req, res) => {
  ...
  try {
    ...
    notifyInboundMessage({phone:sender,body:incomingBody}).catch(err=>console.error('No se pudo avisar mensaje entrante:',err?.message||err));
    ...
    if (!reply?.silent && (reply?.body || reply?.mediaUrl || (Array.isArray(reply?.mediaUrls)&&reply.mediaUrls.length) || (Array.isArray(reply?.messages)&&reply.messages.length))) {
      await recordMessage({
        phone: sender,
        direction: 'outbound',
        source: routeSource,
...
app.listen(PORT, () => {
  ...
  // Twilio queda como respaldo del historial: recupera mensajes aunque un webhook
  // o una escritura de Realtime Database haya fallado temporalmente.
  setTimeout(()=>syncTwilioRecent({limit:1000,force:true}).catch(()=>{}),10000);
  const syncTimer=setInterval(()=>syncTwilioRecent({limit:350,force:true}).catch(()=>{}),2*60*1000);
